<div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-slate-500 mt-1">Gestión de clientes en MySQL</p>
        </div>
        <div class="flex gap-2">
            ...
            <button wire:click="openModal" class="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition">Nuevo cliente</button>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-4 mb-6">
        <div class="relative">
            ...
            <input wire:model.live="search" type="text" placeholder="Buscar clientes..." class="w-full md:w-1/3 pl-10 pr-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
        </div>
    </div>
        ...
        <table class="min-w-full divide-y divide-slate-200">
            <thead class="bg-slate-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Teléfono</th>
                    <th class="px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Dirección</th>
            ...
            <tbody class="divide-y divide-slate-200">
                @forelse($clientes as $cliente)
                    <tr class="hover:bg-slate-50 transition">
                        <td class="px-6 py-4 text-sm font-medium text-slate-900">{{ $cliente->nombre }} {{ $cliente->apellido }}</td>
                        <td class="px-6 py-4 text-sm text-slate-500">{{ $cliente->email }}</td>
                        <td class="px-6 py-4 text-sm text-slate-500">{{ $cliente->telefono ?? '-' }}</td>
                        <td class="px-6 py-4 text-sm text-slate-500">{{ Str::limit($cliente->direccion, 30) ?? '-' }}</td>
                        <td class="px-6 py-4 text-right">
                            <button wire:click="edit({{ $cliente->id }})" class="px-3 py-1.5 text-xs font-medium text-blue-600 hover:bg-blue-50 rounded-lg transition">Editar</button>
                            <button wire:click="delete({{ $cliente->id }})" wire:confirm="¿Eliminar este cliente?" class="px-3 py-1.5 text-xs font-medium text-red-600 hover:bg-red-50 rounded-lg transition">Eliminar</button>
                        </td>
                    </tr>
                @empty
                    <tr>
    ...
    <div class="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md">
            <div class="px-6 py-4 border-b border-slate-200">
                <h2 class="text-lg font-semibold text-slate-900">{{ $editId ? 'Editar' : 'Nuevo' }} cliente</h2>
            </div>
            <form wire:submit="save" class="p-6 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input wire:model="apellido" type="text" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
                        @error('apellido') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Email</label>
                    <input wire:model="email" type="email" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
                    @error('email') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Teléfono</label>
                    <input wire:model="telefono" type="text" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Dirección</label>
                    <textarea wire:model="direccion" rows="2" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition resize-none"></textarea>
                </div>
    ...
    <div class="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[80vh] flex flex-col">
            <div class="px-6 py-4 border-b border-slate-200">
                <h2 class="text-lg font-semibold text-slate-900">Clientes eliminados</h2>
            </div>
            <div class="flex-1 overflow-y-auto p-6">
                ...
                @else
                    <div class="text-center py-8">
                        ...
                        <p class="text-slate-500">No hay clientes eliminados</p>
                    </div>
                @endif
            </div>
